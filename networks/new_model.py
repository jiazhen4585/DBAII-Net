...
class my_model(nn.Module):
    ...
    def forward(self, x1, x2):    # x1=(2,1,64,128,128)  x2=(2,1,64,128,128)
        ...
        x1 = self.layer2(x1)    # x1=(2,128,16,32,32)

        x2 = self.conv_stem(x2)    # x2=(2,64,32,64,64)

        x2 = self.layer1(x2)    # x2=(2,64,32,64,64)
        x2 = self.layer2(x2)    # x2=(2,128,16,32,32)

        x = self.cross_attention(x1, x2)    # x=(2,128,16,32,32)

        x = self.layer3(x)    # x=(2,256,8,16,16)
        x = self.layer4(x)
...
def generate_model(model_depth,  n_input_channels=1, n_classes=2, **kwargs):
    ...
    if model_depth == 10:
        model = my_model(BasicBlock, [1, 1, 1, 1], get_inplanes(), n_input_channels=n_input_channels, n_classes=n_classes, **kwargs)
    elif model_depth == 18:
        model = my_model(BasicBlock, [2, 2, 2, 2], get_inplanes(), n_input_channels=n_input_channels, n_classes=n_classes, **kwargs)
    elif model_depth == 34:
        model = my_model(BasicBlock, [3, 4, 6, 3], get_inplanes(), n_input_channels=n_input_channels, n_classes=n_classes, **kwargs)
    elif model_depth == 50:
        model = my_model(Bottleneck, [3, 4, 6, 3], get_inplanes(), n_input_channels=n_input_channels, n_classes=n_classes, **kwargs)
    elif model_depth == 101:
        model = my_model(Bottleneck, [3, 4, 23, 3], get_inplanes(), n_input_channels=n_input_channels, n_classes=n_classes, **kwargs)
    elif model_depth == 152:
        model = my_model(Bottleneck, [3, 8, 36, 3], get_inplanes(), n_input_channels=n_input_channels, n_classes=n_classes, **kwargs)
    elif model_depth == 200:
        model = my_model(Bottleneck, [3, 24, 36, 3], get_inplanes(), n_input_channels=n_input_channels, n_classes=n_classes, **kwargs)
