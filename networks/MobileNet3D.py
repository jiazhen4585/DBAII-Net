...
# Networks for Mobile Vision Applications" for more details.
# -----------------------------------------------------------
...
class Block(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1):
        ...
        self.conv1 = nn.Conv3d(in_planes, in_planes, kernel_size=3,
...
if __name__ == '__main__':
    # model = get_model(num_classes=600, sample_size=112, width_mult=1., in_channels=2)
    model = generate_model(num_classes=600, sample_size=224, width_mult=1., in_channels=2)
    # model = model.cuda()
    # model = nn.DataParallel(model, device_ids=None)
    print(model)

    # input_var = Variable(torch.randn(8, 2, 16, 112, 112))
    input_var = Variable(torch.randn(8, 2, 224, 224, 16))
