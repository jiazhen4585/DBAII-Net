...
def generate_model(blocks_args, global_params, in_channels):
    # model = EfficientNet3D(blocks_args=blocks_args, global_params=global_params, in_channels=in_channels)
    model = EfficientNet3D.from_pretrained('efficientnet-b0')
...
if __name__ == "__main__":
    # model = generate_model(model_depth=model_depth, n_input_channels=3, num_classes=2)
    model = generate_model(blocks_args=None, global_params=None, in_channels=3)
    sample_input = torch.randn(2, 3, 224, 224, 32)
