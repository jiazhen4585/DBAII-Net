...
class Bottleneck(nn.Module):
    def __init__(self, in_planes, out_planes, stride, groups):
        ...
        if stride == 2:
            self.shortcut = nn.AvgPool3d(kernel_size=(3,3,2), stride=2, padding=(1,1,0))
...
class ShuffleNet(nn.Module):
    def __init__(self, groups, width_mult=1, num_classes=400, in_channels=1):
        ...
        self.conv1   = conv_bn(in_channels, self.in_planes, stride=(2,2,1))
...
if __name__ == "__main__":
    model = generate_model(groups=3, num_classes=600, width_mult=1, in_channels=2)
    # model = model.cuda()
