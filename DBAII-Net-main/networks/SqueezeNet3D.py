    ...
    def __init__(self,
        ...
        if version == 1.0:
            self.features = nn.Sequential(
                nn.Conv3d(in_channels, 96, kernel_size=7, stride=(2, 2, 1), padding=(3, 3, 3)),
                nn.BatchNorm3d(96),
        ...
        if version == 1.1:
            self.features = nn.Sequential(
                nn.Conv3d(in_channels, 64, kernel_size=3, stride=(2, 2, 1), padding=(1, 1, 1)),
                nn.BatchNorm3d(64),
                nn.ReLU(inplace=True),
                nn.MaxPool3d(kernel_size=3, stride=2, padding=1),
                Fire(64, 16, 64, 64),
        ...
        self.classifier = nn.Sequential(
            ...
            final_conv,
            nn.ReLU(inplace=True),
            nn.AvgPool3d((last_size, last_size, last_duration), stride=1)
        )
...
if __name__ == '__main__':
    # model = SqueezeNet(version=1.1, sample_size=96, sample_duration=16, num_classes=600, in_channels=2)
    model = SqueezeNet(version=1.1, sample_size=224, sample_duration=32, num_classes=600, in_channels=2)
    # model = model.cuda()
    # model = nn.DataParallel(model, device_ids=None)
    print(model)

    # input_var = Variable(torch.randn(8, 2, 16, 112, 112))
    # input_var = Variable(torch.randn(8, 2, 112, 112, 16))
    input_var = Variable(torch.randn(8, 2, 224, 224, 32))
